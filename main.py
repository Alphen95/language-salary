...
def main():
    ...
    for job in jobs:
        for language in languages:
            if language in job["name"]:
                ...
                salaries[language]["hh"][1] += predict_salary(job["salary"]["from"],job["salary"]["to"]) if job["salary"]["currency"] == "RUR" else 0
                salaries[language]["hh"][2] += 1 if job ["salary"]["currency"] == "RUR" else 0
    ...
    for language in languages:
        for i in range(0,20):
            ...
            for obj in superjob_vacancies["objects"]:jobs.append(obj)
    for job in jobs:
        for language in languages:
            if language in job["profession"]:
                salaries[language]["sj"][0] += 1
                salaries[language]["sj"][1] += predict_salary(job["payment_from"] if job["currency"] =="rub" else None,job["payment_to"] if job["currency"] =="rub" else None)
                salaries[language]["sj"][2] += 1 if job["currency"] == "rub" else 0
    for language in salaries.items():
        language[1]["hh"][1] //= language[1]["hh"][0] if language[1]["hh"][0] > 0 else 1
        language[1]["sj"][1] //= language[1]["sj"][0] if language[1]["sj"][0] > 0 else 1
    ...
    for language in salaries.items():
        table_data_hh.append([language[0],language[1]["hh"][0],language[1]["hh"][2],language[1]["hh"][1]])
